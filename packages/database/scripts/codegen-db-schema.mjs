...
const outFile = path.join(packageDir, "src/schema.ts");
const envFiles = [
  path.join(packageDir, ".env.local"),
  path.join(repoRoot, "apps/backend/.env.local"),
];
const configFile = path.join(packageDir, "scripts/codegen-db.config.json");
const codegenBin = path.join(packageDir, "node_modules/.bin/kysely-codegen");
...
if (tables.length === 0) {
  ...
  console.error("Temporary override: pnpm database:codegen -- qywx_account qywx_message");
  process.exit(1);
}
...
function getDatabaseUrl() {
  ...
  if (!envFile) {
    throw new Error(`DATABASE_URL is missing. Create ${envFiles[0]} first.`);
  }
