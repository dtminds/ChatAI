...
- `deploy/workflow-worker.Dockerfile`：使用 Debian glibc 镜像构建并运行独立 Workflow Worker，以兼容 `pulsar-client` 原生扩展；镜像暴露 3002 健康检查端口。
...
`/healthz` 只表示进程存活。`/readyz` 要求数据库、Broker 和所有已启用角色均就绪。TKE 建议使用：
