...
4. 在应用内按 UID 去重。
5. 按 `idempotency_key` 升序排列批量 values，再以固定幂等键合并 `sessionize_uid` 任务。
...
第 5 步内部也必须保持稳定的唯一键顺序。发现批次与到期扫描会并发执行批量和单条 `INSERT ... ON DUPLICATE KEY UPDATE`；批量语句若先锁定较大的已有唯一键，再向其前方 gap 插入较小键，会与等待该已有键的单条语句形成反向等待。排序只改变 values 顺序，不改变 UID 去重结果、任务语义、SQL 数量或事务边界。
