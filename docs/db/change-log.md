...
- 请求先复用已有结果；未命中时直接调用模型，并通过唯一键和 `INSERT IGNORE` 保留首个写入结果。并发重复模型调用可接受，不引入运行态、租约或轮询接管。
...
- 若测试环境已按本分支早期版本创建含租约字段的表，先删除未完成记录，再执行：

```sql
DELETE FROM xy_wap_embed_chat_agent_preflight
WHERE outcome IS NULL OR reasoning_summary IS NULL OR source IS NULL;

ALTER TABLE xy_wap_embed_chat_agent_preflight
  DROP COLUMN status,
  ...
  MODIFY COLUMN outcome VARCHAR(32) NOT NULL COMMENT '预判结果：no_response_needed、response_needed',
  MODIFY COLUMN reasoning_summary VARCHAR(255) NOT NULL COMMENT '面向客服的对话进展摘要',
  MODIFY COLUMN source VARCHAR(16) NOT NULL COMMENT '结果来源：model、fallback';
```
