...
保存时校验绝对时间完整且开始不晚于结束，订单金额最低值不大于最高值。相对时间和绝对时间均从第 361 天开始判为无效：时间点达到保存时刻前第 361 天，或时间跨度达到 361 天时拦截，因此完整的第 360 个自然日（最多 360 天 23 小时 59 分）允许保存。用户提示仍表述为“时间不能早于 360 天前”或“时间跨度不能超过 360 天”。Runtime 对非动态时间重复执行该边界检查，防止绝对时间配置随工作流长期运行而过期。
