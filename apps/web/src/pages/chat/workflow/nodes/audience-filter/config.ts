import type {
  WorkflowAudienceFilterMatchMode,
  WorkflowAudienceGroupCreateType,
  WorkflowAudienceGroupListItem,
  WorkflowAudienceGroupSnapshot,
} from "@chatai/contracts";
import {
  WORKFLOW_AUDIENCE_GROUP_CALCULATE_TIME_MAX_LENGTH,
  WORKFLOW_AUDIENCE_GROUP_CONDITION_MAX_COUNT,
  WORKFLOW_AUDIENCE_GROUP_CONDITIONS_MAX_LENGTH,
  WORKFLOW_AUDIENCE_GROUP_CREATE_TYPE_IMPORT,
  WORKFLOW_AUDIENCE_GROUP_CREATE_TYPE_RULE,
  WORKFLOW_AUDIENCE_GROUP_LIST_PAGE_SIZE_MAX,
  WORKFLOW_AUDIENCE_GROUP_MAX_COUNT,
  WORKFLOW_AUDIENCE_GROUP_NAME_MAX_LENGTH,
} from "@chatai/contracts";

export function normalizeWorkflowAudienceFilterMatchMode(
  value: unknown,
): WorkflowAudienceFilterMatchMode {
  return value === "all" || value === "none" ? value : "any";
}

export function normalizeWorkflowAudienceGroup(
  value: unknown,
): WorkflowAudienceGroupSnapshot | undefined {
  if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
  const record = value as Record<string, unknown>;
  const id = record.id;
  const name = typeof record.name === "string" ? record.name.trim() : "";
  if (!Number.isSafeInteger(id) || Number(id) < 1 || !name) return undefined;
  return {
    id: Number(id),
    name: name.slice(0, WORKFLOW_AUDIENCE_GROUP_NAME_MAX_LENGTH),
  };
}

export function toWorkflowAudienceGroupSnapshot(
  group: Pick<WorkflowAudienceGroupListItem, "id" | "name">,
): WorkflowAudienceGroupSnapshot {
  return { id: group.id, name: group.name };
}

export function normalizeWorkflowAudienceGroups(value: unknown): WorkflowAudienceGroupSnapshot[] {
  return uniqueAudienceGroups(value, WORKFLOW_AUDIENCE_GROUP_MAX_COUNT, normalizeWorkflowAudienceGroup);
}

export function normalizeWorkflowAudienceGroupCatalog(
  value: unknown,
): WorkflowAudienceGroupListItem[] {
  return uniqueAudienceGroups(value, WORKFLOW_AUDIENCE_GROUP_LIST_PAGE_SIZE_MAX, normalizeWorkflowAudienceGroupListItem);
}

function normalizeWorkflowAudienceGroupListItem(
  value: unknown,
): WorkflowAudienceGroupListItem | undefined {
  const snapshot = normalizeWorkflowAudienceGroup(value);
  if (!snapshot || !value || typeof value !== "object" || Array.isArray(value)) return undefined;
  const record = value as Record<string, unknown>;
  const group: WorkflowAudienceGroupListItem = { ...snapshot };
  const conditions = normalizeConditions(record.conditions);
  if (conditions) group.conditions = conditions;
  const createType = normalizeWorkflowAudienceGroupCreateType(record.createType);
  if (createType != null) group.createType = createType;
  const groupNum = normalizeNonNegativeInteger(record.groupNum);
  if (groupNum != null) group.groupNum = groupNum;
  const peopleCalculateTime = typeof record.peopleCalculateTime === "string"
    ? record.peopleCalculateTime.trim().slice(0, WORKFLOW_AUDIENCE_GROUP_CALCULATE_TIME_MAX_LENGTH)
    : "";
  if (peopleCalculateTime) group.peopleCalculateTime = peopleCalculateTime;
  return group;
}

function normalizeConditions(value: unknown) {
  const parts: string[] = [];
  const source = typeof value === "string"
    ? value.split(/\r?\n/)
    : Array.isArray(value)
      ? value
      : null;
  if (!source) return undefined;

  for (const item of source) {
    if (typeof item !== "string") continue;
    const part = item.trim().slice(0, WORKFLOW_AUDIENCE_GROUP_CONDITIONS_MAX_LENGTH);
    if (!part) continue;
    parts.push(part);
    if (parts.length >= WORKFLOW_AUDIENCE_GROUP_CONDITION_MAX_COUNT) break;
  }

  return parts.length > 0 ? parts : undefined;
}

function normalizeWorkflowAudienceGroupCreateType(
  value: unknown,
): WorkflowAudienceGroupCreateType | undefined {
  return value === WORKFLOW_AUDIENCE_GROUP_CREATE_TYPE_RULE
    || value === WORKFLOW_AUDIENCE_GROUP_CREATE_TYPE_IMPORT
    ? value
    : undefined;
}

function normalizeNonNegativeInteger(value: unknown) {
  if (typeof value === "number" && Number.isSafeInteger(value) && value >= 0) return value;
  if (typeof value === "string" && value.trim()) {
    const parsed = Number(value.trim());
    if (Number.isSafeInteger(parsed) && parsed >= 0) return parsed;
  }
  return undefined;
}

function uniqueAudienceGroups<T extends WorkflowAudienceGroupSnapshot>(
  value: unknown,
  maxItems: number,
  normalize: (value: unknown) => T | undefined,
): T[] {
  if (!Array.isArray(value)) return [];
  const groups: T[] = [];
  const seen = new Set<number>();
  for (const item of value) {
    const group = normalize(item);
    if (!group || seen.has(group.id)) continue;
    seen.add(group.id);
    groups.push(group);
    if (groups.length >= maxItems) break;
  }
  return groups;
}

export function getWorkflowAudienceFilterMatchModeLabel(
  matchMode: WorkflowAudienceFilterMatchMode,
) {
  if (matchMode === "all") return "满足全部";
  if (matchMode === "none") return "均不包含";
  return "满足任一";
}

export function getWorkflowAudienceFilterMetric(
  matchMode: WorkflowAudienceFilterMatchMode,
  groups: readonly WorkflowAudienceGroupSnapshot[],
) {
  if (groups.length === 0) return "未配置人群包";
  return `${getWorkflowAudienceFilterMatchModeLabel(matchMode)} · ${groups.length} 个人群包`;
}

export function isWorkflowAudienceFilterConfigured(
  groups: readonly WorkflowAudienceGroupSnapshot[],
) {
  return groups.length > 0;
}

export function getWorkflowAudienceGroupRuleDisplay(
  group: Pick<WorkflowAudienceGroupListItem, "conditions" | "createType">,
) {
  if (group.createType === WORKFLOW_AUDIENCE_GROUP_CREATE_TYPE_IMPORT) {
    return { kind: "import" as const };
  }
  if (group.conditions && group.conditions.length > 0) {
    return { kind: "conditions" as const, items: group.conditions };
  }
  return { kind: "empty" as const };
}
