  ...
  async getExecutionLog(input: Parameters<WorkflowDataReader["getExecutionLog"]>[0]): Promise<WorkflowEntryRecordExecutionLog> {
    ...
    const execution = await this.db.selectFrom("xy_wap_embed_workflow_node_execution")
      .select([
        ...
        "error_message",
        "id",
        ...(input.includeInput ? ["input_snapshot_json" as const] : []),
        "node_id",
        "node_kind",
        "output_json",
        "sequence",
    ...
    return {
      ...
      inputAvailable: input.includeInput,
      inputSnapshot: input.includeInput ? parseWorkflowJsonObject(execution.input_snapshot_json) : {},
