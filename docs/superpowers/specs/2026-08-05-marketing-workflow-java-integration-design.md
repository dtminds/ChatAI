...
`message-query` 的 `operation.chatai.message.query@1` 由 Node Workflow Worker 直接只读平台消息事实表实现，不经过 Java Capability。查询先按托管账号解析 `third_userid`，再按租户、平台、`third_user_id`、`third_external_id` 和 `msgtime` 查询私聊消息；平台表不由 Node 写入，也不在本能力中增加或修改索引。
