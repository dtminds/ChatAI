...
CREATE TABLE IF NOT EXISTS xy_wap_embed_insight_sync_cursor (
  ...
  uid BIGINT NULL COMMENT '租户ID，为空表示全局水位',
  cursor_msgtime BIGINT NOT NULL DEFAULT 0 COMMENT '当前同步到的平台消息时间戳',
  cursor_audit_id BIGINT NOT NULL DEFAULT 0 COMMENT '当前同步到的平台消息ID',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_sessionization_config (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
  uid BIGINT NOT NULL COMMENT '租户ID',
  preset VARCHAR(32) NOT NULL COMMENT '切片预设类型',
  idle_timeout_minutes INT NOT NULL COMMENT '空闲关闭时长，单位分钟',
  hard_max_duration_hours INT NOT NULL COMMENT '逻辑会话最长持续时长，单位小时',
  analysis_delay_minutes INT NOT NULL COMMENT '关闭后延迟分析时长，单位分钟',
  late_arrival_window_minutes INT NOT NULL COMMENT '迟到消息窗口，单位分钟',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_logical_session (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
  uid BIGINT NOT NULL COMMENT '租户ID',
  conversation_id BIGINT NOT NULL COMMENT '平台会话ID，关联xy_wap_embed_conversation.id',
  started_at BIGINT NOT NULL COMMENT '逻辑会话开始时间戳',
  ended_at BIGINT NULL COMMENT '逻辑会话结束时间戳',
  last_message_at BIGINT NULL COMMENT '最后一条消息时间戳',
  last_meaningful_message_at BIGINT NULL COMMENT '最后一条有效边界消息时间戳',
  status VARCHAR(32) NOT NULL COMMENT '逻辑会话状态',
  close_reason VARCHAR(64) NULL COMMENT '逻辑会话关闭原因',
  rule_version VARCHAR(64) NOT NULL COMMENT '切片规则版本',
  idle_timeout_minutes INT NOT NULL COMMENT '创建时使用的空闲关闭时长',
  hard_max_duration_hours INT NOT NULL COMMENT '创建时使用的最长持续时长',
  analysis_delay_minutes INT NOT NULL COMMENT '创建时使用的延迟分析时长',
  current_snapshot_id BIGINT NULL COMMENT '当前生效洞察快照ID',
  final_snapshot_id BIGINT NULL COMMENT '最终洞察快照ID',
  message_count INT NOT NULL DEFAULT 0 COMMENT '消息总数',
  customer_message_count INT NOT NULL DEFAULT 0 COMMENT '客户消息数',
  agent_message_count INT NOT NULL DEFAULT 0 COMMENT '客服消息数',
  create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_logical_session_message (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
  uid BIGINT NOT NULL COMMENT '租户ID',
  session_id BIGINT NOT NULL COMMENT '逻辑会话ID',
  conversation_id BIGINT NOT NULL COMMENT '平台会话ID',
  source_message_id BIGINT NOT NULL COMMENT '平台消息ID，关联xy_wap_embed_msg_audit_info.id',
  source_message_time BIGINT NOT NULL COMMENT '平台消息发送时间戳',
  sender_role VARCHAR(32) NOT NULL COMMENT '发送方角色',
  occurred_at BIGINT NOT NULL COMMENT '消息发生时间戳',
  message_type VARCHAR(64) NOT NULL COMMENT '标准化消息类型',
  included_for_ai TINYINT NOT NULL DEFAULT 1 COMMENT '是否进入AI上下文，1是0否',
  meaningful_for_boundary TINYINT NOT NULL DEFAULT 1 COMMENT '是否参与切片边界判断，1是0否',
  create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_analysis_run (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
  session_id BIGINT NOT NULL COMMENT '逻辑会话ID',
  job_id BIGINT NULL COMMENT '关联任务ID',
  mode VARCHAR(32) NOT NULL COMMENT '分析模式',
  analysis_scope VARCHAR(64) NOT NULL COMMENT '分析范围',
  source_message_from BIGINT NULL COMMENT '输入消息起始ID',
  source_message_to BIGINT NULL COMMENT '输入消息结束ID',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_session_insight_snapshot (
  ...
  source_message_high_watermark BIGINT NULL COMMENT '输入消息高水位ID',
  analysis_version VARCHAR(64) NOT NULL COMMENT '分析逻辑版本',
  model_profile_id BIGINT NULL COMMENT '模型配置ID',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_session_insight_current (
  session_id BIGINT PRIMARY KEY COMMENT '逻辑会话ID',
  current_snapshot_id BIGINT NOT NULL COMMENT '当前生效快照ID',
  create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'
) COMMENT='逻辑会话当前洞察快照指针表';

CREATE TABLE IF NOT EXISTS xy_wap_embed_session_summary (
  snapshot_id BIGINT PRIMARY KEY COMMENT '洞察快照ID',
  customer_intent TEXT NOT NULL COMMENT '客户诉求摘要',
  process_summary TEXT NOT NULL COMMENT '处理过程摘要',
  result_summary TEXT NOT NULL COMMENT '当前结果摘要',
  follow_up TEXT NULL COMMENT '跟进建议',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_session_sentiment (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
  snapshot_id BIGINT NOT NULL COMMENT '洞察快照ID',
  polarity VARCHAR(32) NOT NULL COMMENT '情绪极性',
  reason TEXT NOT NULL COMMENT '情绪判断理由',
  confidence DECIMAL(5,4) NULL COMMENT '置信度',
  create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_session_problem_resolution (
  snapshot_id BIGINT PRIMARY KEY COMMENT '洞察快照ID',
  problem_detected TINYINT NOT NULL COMMENT '是否识别到客户问题，1是0否',
  problem_summary TEXT NOT NULL COMMENT '客户问题摘要',
  resolution_status VARCHAR(32) NOT NULL COMMENT '问题解决状态',
  unresolved_reason TEXT NULL COMMENT '未解决或部分解决原因',
  agent_action_summary TEXT NULL COMMENT '客服动作摘要',
  customer_final_state TEXT NULL COMMENT '客户最终状态',
  confidence DECIMAL(5,4) NULL COMMENT '置信度',
  create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'
) COMMENT='客户问题解决判定结果表';
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_session_qa_finding (
  ...
  passed TINYINT NOT NULL COMMENT '是否通过，1通过0未通过',
  reason TEXT NOT NULL COMMENT '判定理由',
  confidence DECIMAL(5,4) NULL COMMENT '置信度',
  create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_session_risk (
  ...
  reason TEXT NOT NULL COMMENT '风险理由',
  confidence DECIMAL(5,4) NULL COMMENT '置信度',
  create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_session_faq_candidate (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
  snapshot_id BIGINT NOT NULL COMMENT '洞察快照ID',
  question TEXT NOT NULL COMMENT '候选问题',
  answer_hint TEXT NOT NULL COMMENT '答案建议',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_insight_evidence (
  ...
  dimension_record_id BIGINT NULL COMMENT '结论记录ID',
  session_id BIGINT NOT NULL COMMENT '逻辑会话ID',
  conversation_id BIGINT NOT NULL COMMENT '平台会话ID',
  source_message_id BIGINT NOT NULL COMMENT '平台消息ID',
  evidence_role VARCHAR(32) NOT NULL COMMENT '证据角色',
  reason TEXT NULL COMMENT '证据说明',
  create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_insight_analysis_policy (
  uid BIGINT PRIMARY KEY COMMENT '租户ID',
  live_analysis_enabled TINYINT NOT NULL DEFAULT 1 COMMENT '是否启用实时分析',
  live_min_new_meaningful_messages INT NOT NULL COMMENT '触发实时分析的最少新增有效消息数',
  live_min_interval_minutes INT NOT NULL COMMENT '实时分析最小间隔分钟数',
  final_analysis_enabled TINYINT NOT NULL DEFAULT 1 COMMENT '是否启用最终分析',
  rule_fallback_enabled TINYINT NOT NULL DEFAULT 1 COMMENT '是否启用规则降级',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_insight_label_config (
  ...
  enabled TINYINT NOT NULL DEFAULT 1 COMMENT '是否启用，1启用0禁用',
  include_in_statistics TINYINT NOT NULL DEFAULT 1 COMMENT '是否纳入统计，1是0否',
  create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_insight_qa_rule_config (
  ...
  judgment_criteria TEXT NULL COMMENT '判定标准',
  positive_examples_json JSON NULL COMMENT '正例JSON',
  negative_examples_json JSON NULL COMMENT '反例JSON',
  enabled TINYINT NOT NULL DEFAULT 1 COMMENT '是否启用，1启用0禁用',
  create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_insight_risk_config (
  ...
  description TEXT NULL COMMENT '风险说明',
  severity VARCHAR(32) NOT NULL COMMENT '严重程度',
  keywords_json JSON NULL COMMENT '关键词JSON',
  priority_boost INT NOT NULL DEFAULT 0 COMMENT '优先级加权',
  unresolved_timeout_minutes INT NULL COMMENT '未解决超时时长，单位分钟',
  enabled TINYINT NOT NULL DEFAULT 1 COMMENT '是否启用，1启用0禁用',
  create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_insight_entity_dictionary (
  ...
  enabled TINYINT NOT NULL DEFAULT 1 COMMENT '是否启用，1启用0禁用',
  include_in_aggregation TINYINT NOT NULL DEFAULT 1 COMMENT '是否纳入聚合，1是0否',
  create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_model_profile (
  id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
  uid BIGINT NULL COMMENT '租户ID，为空表示全局配置',
  ...
  timeout_ms INT NOT NULL COMMENT '调用超时时间，单位毫秒',
