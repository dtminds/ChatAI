...
金额筛选由 Java 按主单实付金额执行，`priceRange` 为字符串数组，包含上下边界。前台配置的最低金额和最高金额均不得超过 `100000`，超限时就地提示并阻止保存；允许 `100000` 和最多两位小数。仅填下限时上限补 `999999`，仅填上限时下限补 `0`，两端均未填时不传 `priceRange`。Node 不再本地筛选或聚合订单。
