...
默认分组不写入 `xy_wap_embed_material_collection_group`。默认分组统一用 `xy_wap_embed_material_collection.group_id = 0` 表示。
...
- 拉取素材列表：按 `bizType`、`groupId` 返回素材。`groupId` 缺省表示所有分组，`0` 表示默认分组。
- 收录消息：入参包含 `bizType`、`msgId`、可选 `groupId`。后端校验消息类型与 `bizType` 匹配后写入 collection。
...
表情点击“收录内容”后直接收录到个人默认分组。

文件、小程序、H5 点击“收录内容”后打开分组选择弹窗。用户可选择默认分组或已有自定义分组后保存。
...
- 表情点击后直接调用收录。
- 文件、小程序、H5 打开分组选择并提交收录。
- composer 工具入口展示素材弹窗。
- 管理态支持删除、置顶、移动分组和分组管理操作。
- 点击素材只触发 alert。
