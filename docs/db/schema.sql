...
CREATE TABLE IF NOT EXISTS xy_wap_embed_analysis_run (
  ...
  token_usage JSON NULL COMMENT '本次分析运行内模型调用Token用量累计',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_session_action_item (
  ...
  KEY idx_ticket_uid_assignee_status_id (uid, assignee_sub_user_id, status, id),
  KEY idx_ticket_uid_conversation_status_id (uid, conversation_id, status, id),
  KEY idx_ticket_uid_conversation_id (uid, conversation_id, id),
  KEY idx_ticket_uid_created_id (uid, create_time, id),
  KEY idx_ticket_uid_status_id (uid, status, id),
  KEY idx_ticket_uid_status_due_at (uid, status, due_at),
  KEY idx_ticket_uid_creator_id (uid, created_by_sub_user_id, id),
  KEY idx_ticket_uid_source_status_id (uid, source_type, status, id)
) COMMENT='工单主表';
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_workflow_definition (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  uid BIGINT UNSIGNED NOT NULL COMMENT '租户ID',
  workflow_type TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT 'Workflow类型：1 ChatAI SOP，2 WeCom SOP，3 Member SOP',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_workflow_revision (
  ...
  uid BIGINT UNSIGNED NOT NULL COMMENT '租户ID',
  workflow_type TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT 'Workflow类型：1 ChatAI SOP，2 WeCom SOP，3 Member SOP',
  subject_type TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '主体类型：1 ChatAI联系人，2 企微客户，3 小程序会员',
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_workflow_node_execution (
  ...
  idempotency_key VARCHAR(512) NOT NULL COMMENT '稳定业务幂等键',
