...
export function parseUserMemoryWorkerRuntimeConfig(env: RuntimeEnv = process.env): UserMemoryWorkerRuntimeConfig {
  ...
  const schedule = env.AGENT_USER_MEMORY_DAILY_TIME?.trim() || "02:00";
  const timezone = env.AGENT_USER_MEMORY_TIMEZONE?.trim() || "Asia/Shanghai";
  const executionMode = env.AGENT_USER_MEMORY_EXECUTION_MODE?.trim() || "sync";
  if (schedule !== "02:00") throw new Error("AGENT_USER_MEMORY_DAILY_TIME must be 02:00");
  if (timezone !== "Asia/Shanghai") throw new Error("AGENT_USER_MEMORY_TIMEZONE must be Asia/Shanghai");
...
export function createUserMemoryWorkerRuntime(input: { db: Kysely<Database>; env?: RuntimeEnv; logger: AppLogger }) {
  ...
  const worker = new UserMemoryWorker({
    db: input.db, logger: input.logger, workerId: `${os.hostname()}:${process.pid}`,
  ...
  const run = async () => {
    ...
    running = true;
    try { await worker.tick(); } catch (error) { input.logger.error({ error }, "Agent user-memory worker tick failed"); } finally { running = false; }
  };
