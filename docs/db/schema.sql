...
CREATE TABLE IF NOT EXISTS xy_wap_embed_workflow_definition (
  ...
  creation_source VARCHAR(32) NOT NULL DEFAULT 'manual' COMMENT '创建来源：manual、template',
  source_template_id BIGINT UNSIGNED NULL COMMENT '来源模板ID',
  source_template_version INT UNSIGNED NULL COMMENT '来源模板版本',
  source_configuration_json JSON NULL COMMENT '应用模板时的配置项快照',
  create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (id),
