...
Adapter 不得将下游原始响应直接写入 Workflow。它必须先按节点输出契约提取后续节点需要的最小 JSON 数据。输出只接受由可枚举数据属性组成的 plain object、数组和 JSON 标量；`undefined`、访问器属性、Symbol key、class instance、非有限数字、稀疏数组和循环引用均视为非法。单节点持久化输出硬上限为 8 KiB，完整 Run Context 硬上限为 128 KiB；非法 JSON、超限输出或追加后超限的 Context 都使当前节点终态失败，且不得保存原始响应或继续重试。
