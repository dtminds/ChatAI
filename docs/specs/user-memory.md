...
12. 每个租户、每个目标自然日只有一条运行；每个客户只调用模型一次，失败后直接结束该运行项，不提供任何重试入口。
...
17. 人工记忆优先。AI 不能修改或删除人工记忆；模型调用期间发生人工修改并导致记忆版本变化时，旧模型结果直接丢弃，运行项进入失败终态。
...
8. 用户可以看到每日运行、入选客户数、会话数、消息数，以及实际新增、更新、删除的记忆数量。
...
功能关闭时仍允许查看和人工维护已有记忆；只停止自动调度和 Worker 自动处理。
...
7. 当天只有一条运行，因此额度天然按 `uid + quota_date` 共享。
...
版本变化时丢弃旧结果并直接结束运行项，不重新调用模型；出现更晚自动日期时直接跳过旧结果。
...
- 每个运行项只允许一次模型调用；请求、输出校验或合并失败后直接进入 `failed`，不得再次调用模型。
- Worker 失租后若发现已提交过模型请求的 `submitted` 项，直接记为失败，不重复提交。
- 提供商轮询必须有最大等待时长，不能无限停留在 `running/waiting/prepared`。
- 所有项终态后，运行聚合为 `succeeded`、`partial`、`failed` 或 `canceled`。
- 运行终态时清除配置 `active_run_id`，后续日期可以继续。
- 终态的成功、失败和跳过计数从运行项当前状态重新聚合；Token 计数记录实际发生的单次模型调用。
...
恢复边界只有运行租约恢复和遗漏每日运行补建；已提交过模型请求的运行项不得再次调用模型。未进入当日快照的会话不会跨日结转。
...
| category | 含义 | 示例 |
| --- | --- | --- |
| `customer_profile` | “稳定属性”：客户或收礼人的稳定背景、生活和使用场景，以及已购、在用或长期使用的品类/型号 | 家中有学龄儿童、常为母亲选购、长期在用 A 型号 |
...
人工维护和 AI 提炼是记忆来源，不是分类。人工新增或编辑可以使用以上三类，并以 `source = manual` 记录来源；AI 只允许新增或更新 `customer_profile`、`preference`，不生成 `recent_intent`。现有 AI `recent_intent` 只读并按原有效期自然过期，AI 不得更新或删除。AI 不得修改或删除任何人工记忆。
...
1. 由客户本人直接表达。
2. 对未来服务仍有使用价值。
3. 遵循“当前客户－单一关系－单一对象”的三元组结构，可以压缩为一个明确短句。
4. 不依赖实时业务状态才能成立。
5. 不属于禁止自动提取的敏感信息。

每条 AI 记忆只记录一个可以独立更新或删除的事实。不同属性、偏好、需求、对象、场景或有效期必须拆分为不同记忆，禁止将多个独立事实合并在同一个 `content` 中。例如“长期使用 A 型号咖啡机，偏好无糖咖啡豆”必须拆成两条记忆。

Prompt 的“不进入记忆”规则和判断示例使用同一组原因标签：`业务系统数据`、`本次限定`、`当下状态`、`短期意图`、`业务处理过程`、`非当前客户事实`、`敏感信息`。示例统一使用“提取/不提取｜原因标签”格式，避免模型自行映射同义但边界模糊的术语。

### 6.3 不应自动保存的内容

- 单次会话摘要或客服处理过程。
- 待办、承诺回访、退款处理等行动项。
- 短期需求、近期购买计划和其它临时意图；需要保留时由人工创建 `recent_intent` 并设置有效期。
- 未结投诉或仍在处理中的诉求；仅在结案后，客户明确的长期避雷或稳定偏好才可归入 `preference`。
- 订单、物流、库存、余额、积分等动态业务状态。
- 单次情绪、质检结论或风险判断。
- 人格、价值或经济能力推断。
- 手机号、收货地址、详细住址、身份证件、银行卡、密码和验证码等个人隐私或安全信息；即使客户表达以后长期使用，也不进入 LLM 记忆。
...
- `content` trim 后非空，最长 100 个字符。Prompt 指引 AI 将内容尽量控制在 50 个字符以内，但 50 字不是服务端拒绝边界。
- 每个 AI 操作引用一个来源会话和其中 1 至 3 个客户证据消息。
- AI 输出不包含有效期字段，AI 新增或更新的分类仅限 `customer_profile`、`preference`，领域层统一保存为 `expiresAt: null`。
- 人工创建或编辑 `recent_intent` 时必须提供晚于当前时间且不超过 180 天的 `expiresAt`；其它人工分类的 `expiresAt` 固定为 `null`。
...
- 所有租户默认关闭。
- `extraction_instruction` 业务输入最长 500 字符，可以留空；数据库列保留 `VARCHAR(2000)`，只用于补充模型重点关注方向。
...
4. 每个 AI 操作必须提供有效的客户 `evidenceMessageIds`；保存平台源消息 ID，`sourceSessionId` 由 Node 根据消息归属推导。证据无法匹配时不得应用本次模型结果。
5. 已存 JSON 非法时不得返回空 JSON 覆盖原数据。
6. Java 直接读表时按 `enabled`、`schemaVersion` 和 `expiresAt` 过滤；Java 只读。
...
CREATE TABLE IF NOT EXISTS xy_wap_embed_agent_user_memory_run (
  ...
  memory_added_count INT UNSIGNED NULL COMMENT '实际新增记忆数',
  memory_updated_count INT UNSIGNED NULL COMMENT '实际更新记忆数',
  memory_removed_count INT UNSIGNED NULL COMMENT '实际删除记忆数',
...
- `customer_limit > 0`。
- `candidate_session_limit = max(200, customer_limit * 2)`，并在创建 run 时快照，运行期间不得修改。
- `candidate_session_count <= candidate_session_limit`。
- `selected_customer_count <= customer_limit`。
- 同一租户、同一自然日只有一条运行。
- 不创建 `retry_of_run_id`，也不提供运行重试。
...
- `add`：新增 AI 记忆。
- `update`：新事实补充或替代同一事实维度的已有 AI 记忆；新旧事实冲突或互斥且存在替代内容时使用。
- `remove`：新证据使已有 AI 记忆明确失效，且没有新事实可以替代；存在替代事实时使用 `update`，不使用 `remove + add`。
- `add/update` 的分类只能是 `customer_profile` 或 `preference`，模型不输出有效期字段；`recent_intent` 仅允许人工维护。
- 模型不输出 `sourceSessionId`；每个操作必须输出 1 至 3 个 `evidenceMessageIds`。Node 会去重、转换数字字符串、限制为 3 条并过滤无法匹配的非客户消息；证据缺失、越界、格式或角色不正确时，本次模型结果无效且不更新记忆。
...
- `update/remove` 的目标不得是历史 AI `recent_intent`；该类条目只读并自然过期。
...
结构、证据、目标或内容校验失败时，整批操作不写入。容量不足不属于模型输出非法，按 §9.4 的确定性规则处理。
...
- 精确去重只是后端兜底。模型在 `add` 前必须比较 `current` 中相同对象、属性、场景和适用范围的记忆；已有 AI 记忆覆盖同一事实维度时使用 `update`，已有 manual 记忆覆盖或冲突时不生成操作。

### 9.4 数量上限

- 先过滤已过期条目，再应用操作。
- 先应用合法的 `remove` 和 `update`，再计算剩余容量。
- 对 `add` 先执行精确去重；如果全部新增候选可以放入剩余容量，则全部应用。
- 如果新增候选数量超过剩余容量，则本次所有 `add` 都不应用，但保留合法的 `remove` 和 `update`；不得按模型返回顺序截取部分新增，也不得自动淘汰既有记忆。
...
模型调用期间发生人工修改时会递增记忆版本；旧模型结果因此触发版本冲突，运行项直接进入 `failed`，不重新准备或调用模型。
...
配置关闭、代次变化或失租时整个事务回滚。版本冲突直接将运行项置为 `failed`；更晚自动日期冲突转为 `skipped/superseded`。
...
进程在 `submitted` 后退出时，新持有者将运行项置为 `failed`，不得重新构建输入或再次调用模型。
...
- 轮询有最大等待时长；超过后运行项直接失败，不允许永久停留在 `waiting` 并阻塞后续自然日。
...
`enabled` 与 `extractionInstruction` 至少传一项，`extractionInstruction` 最长 500 字符。修改提炼指引不创建立即运行，只影响保存后的自动提炼请求。Provider 仅在最终指引 trim 后非空时追加“附加提炼指引”章节，并使用 `<extraction_instruction>...</extraction_instruction>` 包裹实际内容；未配置自定义指引时不拼接该章节、说明或标签。行业模板是前端快捷填充能力，只提供具体行业且只补充可长期复用的信息方向，不提供“通用”模板或近期计划模板；后端只保存用户最终编辑后的文本。
...
| 错误码 | 场景 |
| --- | --- |
| `AGENT_USER_MEMORY_DISABLED` | 功能关闭，不能执行自动运行 |
...
| `AGENT_USER_MEMORY_MODEL_JSON_INVALID` | 模型返回内容无法解析为 JSON |
| `AGENT_USER_MEMORY_MODEL_OPERATIONS_INVALID` | 模型返回的操作数量、目标或组合非法 |
| `AGENT_USER_MEMORY_MODEL_OUTPUT_INVALID` | 旧运行保存的通用模型输出错误，无法还原具体原因 |
| `AGENT_USER_MEMORY_MODEL_REQUEST_ALREADY_SUBMITTED` | 模型请求已提交过，为避免重复调用不再处理 |
| `AGENT_USER_MEMORY_MODEL_SCHEMA_INVALID` | 模型返回 JSON 不符合操作字段契约 |
...
2. owner/admin 可以启停、配置提炼指引和人工增删改。
...
9. 默认每自然日最多 100 个去重客户，每个客户只调用模型一次。
...
4. 人工修改导致版本变化时，旧模型结果直接失败，不重新准备或再次调用模型。
5. 未来新会话再次明确表达同一事实时，AI 可以重新新增。

### 18.5 合并和数量

1. 当前有效记忆总数不超过 20。
2. AI 的 `remove/update` 先执行；全部新增候选无法同时放入剩余容量时，本次所有 `add` 都不应用，不影响合法的 `remove/update`。
3. AI 只生成 `customer_profile` 和 `preference`；人工可以创建有明确过期时间的 `recent_intent`。
4. 历史 AI `recent_intent` 不允许被 AI 更新或删除，按原有效期自然过期。
5. 精确重复按固定规则处理。
6. 所有 AI 操作都能验证到运行项内的客户证据。
7. 空操作也把 `last_auto_quota_date` 单调推进到当前 run 日期。
8. 版本冲突时丢弃旧结果并将运行项置为失败终态，不重建输入或再次调用模型。
