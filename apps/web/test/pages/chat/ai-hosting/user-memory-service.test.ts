...
describe("user memory service adapter", () => {
  ...
  it("uses Agent user-memory endpoints and preserves optimistic versions", async () => {
    ...
    mock.onGet("/server/ai-hosting/user-memory/customers/customer%2F1/items/3/evidence?platform=5").reply(200, { data: { messages: [] }, success: true });
    ...
    mock.onPatch("/server/ai-hosting/user-memory/customers/customer%2F1/items/3?platform=5").reply((config) => [200, { data: JSON.parse(config.data), success: true }]);
    ...
    await getUserMemoryEvidence(5, "customer/1", 3);
    await createUserMemoryItem(5, "customer/1", { category: "manual_note", content: "重点服务", expectedVersion: 2, expiresAt: null });
    await updateUserMemoryItem(5, "customer/1", 3, { category: "manual_note", content: "升级服务", expectedVersion: 3, expiresAt: null });
    await deleteUserMemoryItem(5, "customer/1", 3, { expectedVersion: 4 });
