...
describe("workflow start configuration", () => {
  ...
  it("commits WeCom members only after confirming the picker dialog", async () => {
    ...
    const onNodeChange = vi.fn();
    const node = createStartNode(createStartNodeData("wecom_sop"));
    const wecomMembers: WorkflowWeComMemberResource = {
      memberLimit: 100,
      reload: vi.fn(),
      roots: [
        {
          children: [
            {
              avatarUrl: "https://example.com/zhang-san.png",
              children: [],
              id: "1_201",
              kind: "member",
              title: "张三",
    ...
    const selectedNode = createStartNode({
    ...
    const selectedList = screen.getByRole("list", { name: "已选企微成员" });
    expect(within(selectedList).getByText("张三")).toBeInTheDocument();
    expect(within(selectedList).getByText("李四")).toBeInTheDocument();
    expect(within(selectedList).getByRole("img", { name: "张三头像" })).toBeInTheDocument();

    await user.click(within(selectedList).getByRole("button", { name: "移除 张三" }));
    expect(onNodeChange).toHaveBeenLastCalledWith(expect.objectContaining({
      workUserIds: [202],
    }));
  });
