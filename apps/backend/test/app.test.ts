...
describe("backend app", () => {
  ...
  it("disables request logging for playable voice checks", async () => {
    ...
    expect(shouldDisableRequestLogging({ url: "/api/server/workflows/observability/summary" })).toBe(true);
  ...
  it("rejects oversized smart reply reference-message batches", async () => {
    const { app, authorization } = await createAuthenticatedApp();
    ...
    const response = await app.inject({
      ...
      url: "/api/server/smart-reply/reference-messages",
    });
