...
- When an Agent needs to log in to the ChatAI development environment, read `.agents/skills/chatai-dev-e2e-login/SKILL.md` and follow its login entrypoint and failure handling.
...
- 常规改动优先使用代码路径、Vitest 和 build 验证；明确需要浏览器端登录验证时，遵循 `chatai-dev-e2e-login` Skill。
