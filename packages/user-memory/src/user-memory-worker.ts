...
import { countUserMemoryRunItems, resolveCandidateSessionLimit, resolveTerminalRunStatus, resolveUserMemoryCustomerLimit, sumUserMemoryChanges, USER_MEMORY_PLATFORM, type UserMemoryCustomerLimitResolver } from "./user-memory-policy.js";
...
export function groupCandidateSessions(
  sessions: CandidateSession[],
  platform = USER_MEMORY_PLATFORM,
): UserMemoryCustomerGroup[] {
  ...
  for (const session of sessions) {
    ...
    const group = groups.get(key) ?? { platform, thirdExternalUserId: session.third_external_userid, sessions: [] };
