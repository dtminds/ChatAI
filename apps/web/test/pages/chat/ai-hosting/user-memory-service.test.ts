...
describe("user memory service adapter", () => {
  ...
  it("uses Agent user-memory endpoints and preserves optimistic versions", async () => {
    ...
    mock.onGet("/server/ai-hosting/user-memory/customers?page=1&pageSize=20").reply(200, { data: { items: [], page: 1, pageSize: 20, total: 0 }, success: true });
    ...
    await listUserMemoryCustomers({ page: 1, pageSize: 20 });
