...
- `deploy/backend.Dockerfile`：使用 `node:24-alpine` 构建 backend，执行根脚本 `pnpm backend:build`（会将 `@chatai/contracts` 和 `@chatai/voice-service` 固化到 `apps/backend/node_modules/@chatai`），运行阶段只复制 `apps/backend/dist` 与 `apps/backend/node_modules`，并用 `node apps/backend/dist/server.js` 启动。
