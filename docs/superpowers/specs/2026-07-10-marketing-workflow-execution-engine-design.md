...
- 首次启用产生 Revision 1；首次启用后每次发布产生新的不可变 Revision，运行实例始终固定在进入时的 Revision。
...
- Workflow 首次启用后，每次发布成功都创建新 Revision；Revision 单调递增。
...
- `workflow-task-dlq` 承载达到最大基础设施重投次数的异常消息。
...
6. 首次启用后的发布在事务中插入下一 Revision、更新 `published_revision` 并替换 Trigger Binding；Active 或 Paused 状态保持不变。
...
- 首次启用生成 Revision 1；启用后的每次发布生成新的、不可变且可查询的 Revision。
