...
- `reception/all`：缺少创建时间时服务端补最近 30 个自然日，自定义范围最多 60 个自然日。
...
- “我接待的”和“全部工单”的 COUNT/LIST 始终限制在 30 天默认或最多 60 天的自定义创建时间范围，并使用 `(uid, create_time, id)` 索引收窄候选集。
...
- “我接待的”和“全部工单”创建时间默认近 30 天、不可清空且最多选择 60 天；其他视图允许清空。
- 工单编号/标题/客户名称搜索。
- 默认排序由服务端完成，前端不重排当前页。
- 列表操作只提供当前用户有权执行的命令。
