...
全局发现器由洞察 Worker 的独立调度步骤周期性执行，与 UID 处理、到期扫描和 LLM 分析使用独立的单轮批次或执行时长预算。调度周期使用 Worker 部署配置，默认可以沿用现有 Worker tick，但不得因为某个 UID 或 LLM 长任务而阻塞下一轮发现。单个 tick 默认按每批 1000 条连续 drain，最多处理 20 批；空批次或全局水位锁竞争会提前结束本轮。
...
6. UID 内继续沿用现有消息读取和切片算法；每条消息的 `sessionizeMessage()`、消息归属/会话统计写入和复合水位推进放在同一个受 claim token 保护的小事务中。
7. 现有 `uid + source_message_id` 唯一约束继续作为消息归属幂等防线。
8. 单次抢占设置批次数或执行时长上限，避免单个热点 UID 长时间占用 Worker。
9. claim token 用于任务领取、周期续租、逐消息业务写入与水位推进、会话关闭和完成阶段 fencing；不引入整批大事务、额外锁表或分布式锁。
