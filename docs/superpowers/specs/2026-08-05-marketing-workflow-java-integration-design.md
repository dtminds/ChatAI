...
`message-query` 是 Node Workflow Worker 内部实现，不经过 Java。查询先按托管账号解析 `third_userid`，再按租户、平台、`third_user_id`、`third_external_id` 和 `msgtime` 查询私聊消息；平台表不由 Node 写入，也不在本节点中增加或修改索引。固定时间选择精确到秒，开始时间从该秒的 `000` 毫秒起算，结束时间包含该秒直到 `999` 毫秒；动态时间引用保持原始毫秒精度。
