...
import { applyAiMemoryOperations, emptyUserMemoryDocument, parseUserMemoryDocument } from "./user-memory-domain.js";
import type { UserMemoryInputMessage, UserMemoryProvider } from "./user-memory-provider.js";
import { resolveCandidateSessionLimit } from "./user-memory-service.js";
...
export class UserMemoryWorker {
  ...
  constructor(private readonly input: { db: Kysely<Database>; logger: AppLogger; provider: UserMemoryProvider; workerId: string }) {}
  ...
  private async claimOne(): Promise<Claim | undefined> {
    return this.input.db.transaction().execute(async (trx) => {
      const now = new Date();
      const run = await trx.selectFrom("xy_wap_embed_agent_user_memory_run").selectAll()
        .where((eb) => eb.or([
      ...
      const config = await trx.selectFrom("xy_wap_embed_agent_user_memory_config").selectAll().where("uid", "=", run.uid).forUpdate().executeTakeFirst();
      ...
      await trx.updateTable("xy_wap_embed_agent_user_memory_run").set({ status: "running", locked_by: this.input.workerId, claim_token: token, lease_until: new Date(Date.now() + LEASE_MS), started_at: run.started_at ?? now }).where("id", "=", run.id).execute();
      return { run: { ...run, status: "running", locked_by: this.input.workerId, claim_token: token, lease_until: new Date(Date.now() + LEASE_MS), started_at: run.started_at ?? now }, token };
    });
  }
  ...
  private async selectCandidates(claim: Claim) {
    ...
    const sessions = await this.input.db.selectFrom("xy_wap_embed_logical_session as session")
      ...
      .where("session.uid", "=", claim.run.uid).where("session.ended_at", ">=", range.start).where("session.ended_at", "<", range.end)
      .where("session.ended_at", ">", config.enabled_at ?? Number.MAX_SAFE_INTEGER).where("session.message_count", ">=", 5)
      ...
      .limit(claim.run.candidate_session_limit).execute() as CandidateSession[];
    const groups = groupCandidateSessions(sessions).slice(0, claim.run.customer_limit);
    await this.input.db.transaction().execute(async (trx) => {
      await assertClaim(trx, claim);
      for (const group of groups) {
        await trx.insertInto("xy_wap_embed_agent_user_memory_run_item").values({
  ...
  private async prepareInput(claim: Claim, item: ItemRow) {
    ...
    const document = memory ? parseDocument(memory.memories_json) : emptyUserMemoryDocument();
    ...
    for (const session of eligible) {
      const rows = await this.input.db.selectFrom("xy_wap_embed_logical_session_message as ownership")
        ...
        .select(["ownership.session_id", "ownership.source_message_id", "ownership.source_message_time", "ownership.sender_role", "message.content", "message.msgtype"])
        .where("ownership.session_id", "=", session.id).where("ownership.included_for_ai", "=", 1)
        .orderBy("ownership.source_message_time", "desc").orderBy("ownership.source_message_id", "desc").limit(50).execute();
  ...
  private async mergeResult(claim: Claim, item: ItemRow, prepared: Awaited<ReturnType<UserMemoryWorker["prepareInput"]>> & {}, result: Awaited<ReturnType<UserMemoryProvider["complete"]>>) {
    await this.input.db.transaction().execute(async (trx) => {
      ...
      let memory = await trx.selectFrom("xy_wap_embed_agent_user_memory").selectAll().where("uid", "=", item.uid).where("platform", "=", item.platform).where("third_external_userid", "=", item.third_external_userid).forUpdate().executeTakeFirst();
      if ((memory?.version ?? 0) !== currentItem.base_memory_version || (memory?.manual_updated_at ?? null) !== currentItem.base_manual_updated_at) {
        await trx.updateTable("xy_wap_embed_agent_user_memory_run_item").set({ status: "prepared", next_attempt_at: new Date(), last_error_code: "AGENT_USER_MEMORY_VERSION_CONFLICT" }).where("id", "=", item.id).execute();
        await releaseRun(trx, claim); return;
      }
      if (memory?.last_auto_quota_date && formatShanghaiDate(memory.last_auto_quota_date) > formatShanghaiDate(claim.run.quota_date)) {
      ...
      await trx.updateTable("xy_wap_embed_agent_user_memory_run_item").set({ status: "succeeded", input_tokens: currentItem.input_tokens + result.inputTokens, output_tokens: currentItem.output_tokens + result.outputTokens, finished_at: new Date(), last_error_code: null }).where("id", "=", item.id).execute();
      await trx.updateTable("xy_wap_embed_agent_user_memory_run").set({ input_tokens: claim.run.input_tokens + result.inputTokens, output_tokens: claim.run.output_tokens + result.outputTokens, phase: "merging" }).where("id", "=", claim.run.id).where("claim_token", "=", claim.token).execute();
  ...
  private async failItem(claim: Claim, item: ItemRow, error: unknown) {
    await this.input.db.transaction().execute(async (trx) => {
      await assertClaim(trx, claim);
      ...
      const attempts = current.status === "submitted" ? current.attempt_count : current.attempt_count + 1;
      const terminal = attempts >= MAX_ITEM_ATTEMPTS;
      await trx.updateTable("xy_wap_embed_agent_user_memory_run_item").set({ attempt_count: attempts, status: terminal ? "failed" : "prepared", next_attempt_at: terminal ? null : new Date(Date.now() + RETRY_DELAY_MS), last_error_code: errorCode(error), ...(terminal ? { finished_at: new Date() } : {}) }).where("id", "=", item.id).execute();
      await aggregateRun(trx, claim);
    }).catch((fenceError) => this.input.logger.warn({ error: fenceError, runId: claim.run.id }, "Failed to record user-memory item error"));
  }
...
async function releaseRun(trx: Transaction<Database>, claim: Claim) { await trx.updateTable("xy_wap_embed_agent_user_memory_run").set({ status: "pending", run_after: new Date(), locked_by: null, claim_token: null, lease_until: null }).where("id", "=", claim.run.id).where("claim_token", "=", claim.token).executeTakeFirstOrThrow(); }
async function aggregateRun(trx: Transaction<Database>, claim: Claim) {
  const items = await trx.selectFrom("xy_wap_embed_agent_user_memory_run_item").select(["status"]).where("run_id", "=", claim.run.id).execute();
  ...
  if (items.length > 0 && items.every((i) => TERMINAL_ITEM_STATUSES.includes(i.status))) {
    const status = counts.failure === 0 ? "succeeded" : counts.success > 0 || counts.skipped > 0 ? "partial" : "failed";
    await finishRun(trx, claim, status, counts);
  } else await releaseRun(trx, claim);
}
...
function errorCode(error: unknown) { return error instanceof Error ? error.message.slice(0, 128) : "AGENT_USER_MEMORY_WORKER_FAILED"; }
